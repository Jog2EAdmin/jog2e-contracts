...
contract Sneaker is BEP721, Ownable {
    ...
    function setupManager(address[] memory addrs, bool _status)
        external
        onlyOwner
    {
    ...
    function priceOfBNB() public view returns (uint256) {
        ...
        return uint256(_price).mul(1e10);
    }

    function convertAmountBoxToBNB(uint256 quantity)
        public
        view
        returns (uint256)
    {
    ...
    function buyMulti(uint8 quantity) public payable {
        require(msg.sender.balance > msg.value, "Your balance is not enough");
        require(isOpenINO, "INO had been closed");
        require(
            msg.value >= convertAmountBoxToBNB(quantity),
            "insufficient funds"
        );
        require(quantity > 0, "Quantity at least greater than 1");
        require(
            maxBuy >= addressTotalBoxBuy[msg.sender] + quantity,
            "Please check the maximum box that you can buy"
        );
